...
import { hostInquiryHtml, hostInquirySubject, hostInquiryText, type HostInquiry } from "@/lib/inquiry-email";
import { enforceInquiryRateLimit, markWhatsAppSent, shouldSendWhatsApp } from "@/lib/inquiry-rate-limit";
import { isWhatsAppConfigured, sendHostInquiryWhatsApp } from "@/lib/inquiry-whatsapp";
...
export async function POST(request: Request) {
  ...
  const from = env("RESEND_FROM") || "Dream Stay Budva <onboarding@resend.dev>";
  ...
  const gate = await enforceInquiryRateLimit(request, inquiry);
  if (!gate.ok) {
  ...
  if (isWhatsAppConfigured() && (await shouldSendWhatsApp(inquiry))) {
    try {
      await sendHostInquiryWhatsApp(inquiry);
      await markWhatsAppSent(inquiry);
    } catch (error) {
      console.error("[inquiry] whatsapp", error instanceof Error ? error.message : error);
    }
  }
